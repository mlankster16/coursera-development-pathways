...
- **Header nav:** "Design Guide ↗" is removed from the nav; the Design Guide
  is linked from each page's footer and content. In the design, the current
  page's bold label is wider, so items shifted between pages; each item now
  reserves its bold width and the underline space, so the nav sits in the
  same place on every page at every width.
- **Tablet overflow (681–980px):** the design's shell padding at this range
  made the page 8px wider than the window; the shell is now `border-box`.
